export type Language = 'en' | 'ar' | 'fr';

export const translations = {
  en: {
    nav: {
      home: 'Home',
      shop: 'Shop',
      about: 'About Us',
      contact: 'Contact',
    },
    hero: {
      title: 'Define Your Style',
      subtitle: 'Premium caps for the modern generation.',
      cta: 'Shop Now',
    },
    bestSellers: {
      title: 'Best Sellers',
      subtitle: 'Our most popular designs',
    },
    whyChooseUs: {
      title: 'Why Choose Us?',
      highQuality: 'High Quality',
      fastShipping: 'Fast Shipping',
      securePayment: 'Secure Payment',
      easyReturn: 'Easy Returns',
    },
    testimonials: {
      title: 'Customer Reviews',
    },
    newsletter: {
      title: 'Join Our Newsletter',
      placeholder: 'Enter your email',
      button: 'Subscribe',
    },
    cart: {
      title: 'Your Cart',
      empty: 'Your cart is empty',
      total: 'Total',
      checkout: 'Checkout',
      add: 'Add to Cart',
      buyNow: 'Buy Now',
      orderWhatsApp: 'Order via WhatsApp',
      selectSize: 'Select Size',
    },
    shop: {
      filters: 'Filters',
      price: 'Price',
      color: 'Color',
      type: 'Type',
      bestSelling: 'Best Selling',
      newest: 'Newest',
    },
    product: {
      description: 'Description',
      reviews: 'Reviews',
      selectColor: 'Select Color',
    },
    about: {
      text: 'We are a brand specialized in designing high-quality modern caps for youth and style enthusiasts.',
    },
    footer: {
      privacy: 'Privacy Policy',
      returns: 'Returns Policy',
      copyright: '© HATRIX. All rights reserved.',
    },
  },
  ar: {
    nav: {
      home: 'الرئيسية',
      shop: 'المتجر',
      about: 'من نحن',
      contact: 'تواصل معنا',
    },
    hero: {
      title: 'حدد أسلوبك',
      subtitle: 'قبعات فاخرة للجيل الحديث.',
      cta: 'تسوق الآن',
    },
    bestSellers: {
      title: 'الأكثر مبيعاً',
      subtitle: 'تصاميمنا الأكثر شهرة',
    },
    whyChooseUs: {
      title: 'لماذا تختارنا؟',
      highQuality: 'جودة عالية',
      fastShipping: 'شحن سريع',
      securePayment: 'دفع آمن',
      easyReturn: 'إرجاع سهل',
    },
    testimonials: {
      title: 'آراء العملاء',
    },
    newsletter: {
      title: 'اشترك في نشرتنا البريدية',
      placeholder: 'أدخل بريدك الإلكتروني',
      button: 'اشتراك',
    },
    cart: {
      title: 'سلة المشتريات',
      empty: 'سلة المشتريات فارغة',
      total: 'المجموع',
      checkout: 'الدفع',
      add: 'إضافة للسلة',
      buyNow: 'شراء الآن',
      orderWhatsApp: 'اطلب عبر الواتساب',
      selectSize: 'اختر المقاس',
    },
    shop: {
      filters: 'الفلاتر',
      price: 'السعر',
      color: 'اللون',
      type: 'النوع',
      bestSelling: 'الأكثر مبيعاً',
      newest: 'الأحدث',
    },
    product: {
      description: 'الوصف',
      reviews: 'التقييمات',
      selectColor: 'اختر اللون',
    },
    about: {
      text: 'نحن علامة متخصصة في تصميم قبعات عصرية عالية الجودة للشباب ومحبي الأناقة.',
    },
    footer: {
      privacy: 'سياسة الخصوصية',
      returns: 'سياسة الاسترجاع',
      copyright: '© HATRIX. جميع الحقوق محفوظة.',
    },
  },
  fr: {
    nav: {
      home: 'Accueil',
      shop: 'Boutique',
      about: 'À Propos',
      contact: 'Contact',
    },
    hero: {
      title: 'Définissez Votre Style',
      subtitle: 'Des casquettes premium pour la génération moderne.',
      cta: 'Acheter Maintenant',
    },
    bestSellers: {
      title: 'Meilleures Ventes',
      subtitle: 'Nos designs les plus populaires',
    },
    whyChooseUs: {
      title: 'Pourquoi Nous Choisir ?',
      highQuality: 'Haute Qualité',
      fastShipping: 'Livraison Rapide',
      securePayment: 'Paiement Sécurisé',
      easyReturn: 'Retour Facile',
    },
    testimonials: {
      title: 'Avis Clients',
    },
    newsletter: {
      title: 'Rejoignez Notre Newsletter',
      placeholder: 'Entrez votre email',
      button: 'S\'abonner',
    },
    cart: {
      title: 'Votre Panier',
      empty: 'Votre panier est vide',
      total: 'Total',
      checkout: 'Paiement',
      add: 'Ajouter au Panier',
      buyNow: 'Acheter Maintenant',
      orderWhatsApp: 'Commander via WhatsApp',
      selectSize: 'Choisir la Taille',
    },
    shop: {
      filters: 'Filtres',
      price: 'Prix',
      color: 'Couleur',
      type: 'Type',
      bestSelling: 'Meilleures Ventes',
      newest: 'Nouveautés',
    },
    product: {
      description: 'Description',
      reviews: 'Avis',
      selectColor: 'Choisir la Couleur',
    },
    about: {
      text: 'Nous sommes une marque spécialisée dans la conception de casquettes modernes de haute qualité pour les jeunes et les passionnés de style.',
    },
    footer: {
      privacy: 'Politique de Confidentialité',
      returns: 'Politique de Retour',
      copyright: '© HATRIX. Tous droits réservés.',
    },
  },
};
